...
const Projects: React.FC = () => {
  ...
  const filteredProjects = useMemo(() => projects.filter((p) => !p.title.toLowerCase().includes("trading bot")), []);
  ...
  return (
    <>
        ...
        <div ref={containerRef} className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-10 relative z-10">
          <RevealWrapper animation="fadeIn" delay={100} className="text-center mb-8 sm:mb-12 md:mb-16 lg:mb-20" triggerOnce={false} threshold={0.3}>
            <div className="inline-flex items-center gap-2 sm:gap-3 px-3 sm:px-4 md:px-6 py-2 sm:py-3 md:py-4 bg-surface/10 backdrop-blur-xl border border-borderc/20 rounded-full mb-4 sm:mb-6 md:mb-8 shadow-lg hover:shadow-xl transition-all duration-500 hover:bg-surface/15 group">
              ...
              <span className="text-xs sm:text-sm font-semibold text-content/70 group-hover:text-content/80 transition-colors duration-300 tracking-wide">FEATURED PROJECTS</span>
                  ...
                  <div key={project.title} className="w-full">
                    ...
                    >
                      <RevealWrapper animation="slideUp" delay={500 + index * 200} triggerOnce={false} threshold={0.25} className="[display:contents]">
                        {!isMobile && !isTablet && (
                          <div className="absolute inset-0 pointer-events-none bg-gradient-to-r from-transparent via-[rgb(255_255_255/0.04)] to-transparent opacity-0 group-hover:opacity-100 duration-700 -skew-x-12 -translate-x-full group-hover:translate-x-[300%] transition-all ease-out" />
                        )}
                        <div className={`absolute inset-0 pointer-events-none bg-gradient-to-br ${colors.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
                        ...
                        <div className="relative h-36 sm:h-48 md:h-60 lg:h-72 xl:h-80 overflow-hidden">
                            ...
                            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                              {particles.map((p, i) => (
                                <div
                                  ...
                                  style={{ left: p.left, top: p.top, animation: `pulse ${p.duration} ease-in-out infinite`, animationDelay: p.delay, transform: `scale(${p.scale})` }}
                                />
                              ))}
                            </div>
                          )}

                          <RevealWrapper animation="slideLeft" delay={200} className="[display:contents]" triggerOnce={false} threshold={0.25}>
                            <div className="absolute top-2 sm:top-3 md:top-4 lg:top-6 xl:top-8 right-2 sm:right-3 md:right-4 lg:right-6 xl:right-8 z-10">
                              <div
                                className={`flex gap-1.5 sm:gap-2 md:gap-3 ${isMobile || isTablet ? "opacity-100" : "opacity-0 group-hover:opacity-100"} transition-all duration-400 ${
                                  isMobile || isTablet ? "translate-y-0" : "translate-y-2 group-hover:translate-y-0"
                                }`}
                              >
                                <a
                                  href={project.github}
                                ...
                                >
                                  <Github size={isMobile ? 14 : isTablet ? 16 : 18} className="text-content/80 group-hover/link:text-content transition-colors duration-200" />
                                </a>
                                {project.demo !== "#" && (
                                  <a
                                    href={project.demo}
                                  ...
                                  >
                                    <ExternalLink size={isMobile ? 14 : isTablet ? 16 : 18} className="text-content/80 group-hover/link:text-content transition-colors duration-200" />
                                  </a>
                                )}
                              </div>
                            </div>
                          ...
                          <RevealWrapper animation="rotateIn" delay={250} className="[display:contents]" triggerOnce={false} threshold={0.25}>
                            <div className="absolute bottom-2 sm:bottom-3 md:bottom-4 lg:bottom-6 xl:bottom-8 left-2 sm:left-3 md:left-4 lg:left-6 xl:left-8 z-10">
                              <div className={`w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 bg-surface/40 backdrop-blur-xl rounded-lg sm:rounded-xl md:rounded-2xl border border-borderc/30 flex items-center justify-center group-hover:bg-surface/50 group-hover:border-borderc/40 transition-all duration-400 group-hover:scale-105 shadow-lg ${colors.glow}`}>
                                <ProjectIcon className={`w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 lg:w-8 lg:h-8 text-content/80 group-hover:text-content ${colors.text} transition-all duration-300`} />
                              </div>
                            </div>
                          </RevealWrapper>

                          <RevealWrapper animation="slideUp" delay={300} className="[display:contents]" triggerOnce={false} threshold={0.25}>
                            <div className="absolute bottom-2 sm:bottom-3 md:bottom-4 lg:bottom-6 xl:bottom-8 right-2 sm:right-3 md:right-4 lg:right-6 xl:right-8 z-10">
                              <div className={`${isMobile || isTablet ? "opacity-100" : "opacity-50 group-hover:opacity-100"} transition-all duration-400 group-hover:scale-105`}>
                                <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 px-2 sm:px-2.5 md:px-3 lg:px-4 py-1 sm:py-1.5 md:py-2 bg-surface/40 backdrop-blur-xl rounded-md sm:rounded-lg md:rounded-xl border border-borderc/30 shadow-lg min-h-[32px]">
                                  ...
                                  <span className="text-xs sm:text-sm font-medium text-content/70 whitespace-nowrap">View Details</span>
                        ...
                        <div className="p-3 sm:p-4 md:p-6 lg:p-8 xl:p-10">
                          <div className="flex items-start justify-between mb-4 sm:mb-6 md:mb-8">
                            <div className="flex-1 min-w-0">
                              <h3 className={`text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-content ${colors.text} transition-colors duration-300 mb-2 sm:mb-3 md:mb-4 leading-tight`}>{project.title}</h3>
                              <p className="text-content/70 leading-relaxed group-hover:text-content/80 transition-colors duration-300 text-xs sm:text-sm md:text-base lg:text-lg">{project.description}</p>
                            </div>
                          </div>
                          ...
                          <StaggeredReveal animation="slideUp" staggerDelay={80} baseDelay={600 + index * 200} className="flex flex-wrap gap-1.5 sm:gap-2 md:gap-3" triggerOnce={false} threshold={0.25}>
                            {project.tech.map((tech) => (
                              <span key={tech} className={`px-2 sm:px-2.5 md:px-3 lg:px-4 py-1 sm:py-1.5 md:py-2 bg-surface/20 text-content/80 rounded-md sm:rounded-lg text-xs sm:text-sm md:text-base border border-borderc/30 hover:bg-surface/30 hover:text-content ${colors.tech} transition-all duration-300 transform hover:scale-105 backdrop-blur-sm font-medium`}>
                                {tech}
                              </span>
                            ))}
                          </StaggeredReveal>
          ...
          <RevealWrapper animation="fadeIn" delay={1000 + filteredProjects.length * 200} className="text-center mt-12 sm:mt-16 md:mt-20 lg:mt-24" triggerOnce={false} threshold={0.3}>
            <div className="inline-flex items-center gap-2 sm:gap-3 md:gap-4 px-4 sm:px-6 md:px-8 py-2 sm:py-3 md:py-4 bg-surface/15 backdrop-blur-xl border border-borderc/20 rounded-full hover:bg-surface/25 hover:border-borderc/30 transition-all duration-500 group cursor-pointer shadow-lg hover:shadow-xl">
              ...
              <span className="text-xs sm:text-sm md:text-base lg:text-lg text-content/70 group-hover:text-content/80 transition-colors duration-300 font-medium">Exploring new horizons</span>
      ...
      {selectedProject !== null && (
        <div
          className={`fixed inset-0 z-[9999] flex items-center justify-center px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-6 transition-opacity duration-200 ${
            modalAnimation === "enter" ? "opacity-100" : modalAnimation === "exit" ? "opacity-0" : "opacity-100"
          }`}
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <div
            className={`relative w-full max-w-[min(95vw,${isMobile ? "100%" : isTablet ? "85vw" : "60rem"})] max-h-[min(85dvh,${isMobile ? "90dvh" : "780px"})] grid grid-rows-[auto_minmax(0,1fr)_auto] bg-surface/90 backdrop-blur-2xl rounded-xl sm:rounded-2xl border border-borderc/30 shadow-[0_10px_50px_-12px_rgb(0_0_0/0.55)] ring-1 ring-[rgb(var(--accent)/0.12)] transition-[opacity,transform] duration-200 ease-out will-change-transform isolate ${
              modalAnimation === "enter" ? "opacity-100 translate-y-0" : modalAnimation === "exit" ? "opacity-0 translate-y-2" : "opacity-100 translate-y-0"
            }`}
            onClick={(e) => e.stopPropagation()}
            style={{ ["--accent-local" as any]: variantTriplet(filteredProjects[selectedProject].color) }}
          >
            {(() => {
                ...
                <>
                  <div className="relative overflow-hidden bg-surface/50 border-b border-borderc/30">
                    <button
                      ...
                      aria-label="Close"
                    >
                      <X size={isMobile ? 16 : 18} className="text-content/80" />
                    </button>
                    ...
                    <div className="p-3 sm:p-4 md:p-6 lg:p-8 pr-12 sm:pr-16">
                      <div className="flex items-start gap-2 sm:gap-3 md:gap-4 lg:gap-5">
                        ...
                        <div className="min-w-0 flex-1">
                          <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-content leading-tight mb-1 sm:mb-2">{project.title}</h2>
                          <p className="text-content/80 leading-relaxed text-xs sm:text-sm md:text-base mb-2 sm:mb-3 md:mb-4">{details.fullDescription}</p>
                  ...
                  <div className="min-h-0 overflow-y-auto overscroll-contain custom-scrollbar">
                    <div className="p-3 sm:p-4 md:p-6 lg:p-8 space-y-3 sm:space-y-4 md:space-y-6">
                      {details.performance && (
                        <div className={`p-3 sm:p-4 md:p-5 rounded-lg sm:rounded-xl border ${colors.modalAccent} backdrop-blur-xl`}>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 md:gap-4">
                            {Object.entries(details.performance).map(([k, v]) => (
                              <div key={k} className="text-center p-2 sm:p-3 md:p-4 bg-surface/20 rounded-md sm:rounded-lg border border-borderc/20">
                                <div className="text-base sm:text-lg md:text-xl font-bold text-content mb-1">{v}</div>
                                <div className="text-content/60 text-xs sm:text-sm capitalize">{k.replace(/([A-Z])/g, " $1").trim()}</div>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      <div className={`p-3 sm:p-4 md:p-5 rounded-lg sm:rounded-xl border ${colors.modalAccent} backdrop-blur-xl`}>
                        ...
                        <ul className="grid gap-2">
                          {details.features.map((f, i) => (
                            <li key={i} className="flex items-start gap-2 sm:gap-3 p-2 sm:p-3 rounded-md sm:rounded-lg bg-surface/20 border border-borderc/15">
                              ...
                              <span className="text-content/80 text-xs sm:text-sm md:text-base">{f}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                      ...
                      <div className={`p-3 sm:p-4 md:p-5 rounded-lg sm:rounded-xl border ${colors.modalAccent} backdrop-blur-xl`}>
                        ...
                        <div className="flex flex-wrap gap-1 sm:gap-1.5 md:gap-2">
                          {project.tech.map((t) => (
                            <span key={t} className={`px-2 sm:px-3 py-1 sm:py-1.5 bg-surface/25 text-content/80 rounded-md sm:rounded-lg text-xs sm:text-sm border border-borderc/30 ${colors.tech}`}>
                              {t}
                            </span>
                          ))}
                        </div>
                  ...
                  <div className="border-t border-borderc/30 p-3 sm:p-4 md:p-5 bg-surface/50">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 text-content/60">
                        ...
                        <span className="text-xs sm:text-sm font-medium hidden sm:inline">Press ESC to close</span>
                        <span className="text-xs font-medium sm:hidden">Tap outside to close</span>
                      </div>
                      <button onClick={handleCloseModal} className={`px-3 sm:px-4 py-1.5 sm:py-2 border ${colors.buttonSecondary} rounded-md sm:rounded-lg font-semibold text-xs sm:text-sm min-h-[36px] active:scale-[0.98] transition-all`}>
                        Close
                      </button>
                    </div>
                  </div>
      ...
      <style>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: rgb(var(--text)/0.2); border-radius: 4px; border: 1px solid transparent; background-clip: padding-box; }
        ...
        @media (max-width: 1024px) { .group{ touch-action: manipulation; } button,a{ min-height:36px; min-width:36px; } }
        @media (prefers-reduced-motion: reduce) { *,*::before,*::after{ animation-duration:0.01ms !important; animation-iteration-count:1 !important; transition-duration:0.01ms !important; } }
        @media (max-width: 480px) { .custom-scrollbar::-webkit-scrollbar{ width:2px; } }
        @media (orientation: landscape) and (max-height: 600px) { .fixed.inset-0{ padding:8px !important; } }
        @supports (height: 100dvh) { .fixed.inset-0{ height:100dvh; } }
        @supports not (height: 100dvh) { .fixed.inset-0{ height:100vh; } }
      `}</style>
    </>
  );
};
